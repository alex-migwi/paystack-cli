...
export async function updateWebhookUrl(token, integration, webhookUrl, domain = 'test') {
  if (!token) return null;
  try {
    const field = domain === 'test' ? 'test_webhook_endpoint' : 'live_webhook_endpoint';
    const resp = await axios.put(
      'https://api.paystack.co/integration/webhooks',
      { [field]: webhookUrl, integration },
      { headers: { Authorization: 'Bearer ' + token, 'jwt-auth': true } }
    );
    return resp.data;
  } catch (err) {
